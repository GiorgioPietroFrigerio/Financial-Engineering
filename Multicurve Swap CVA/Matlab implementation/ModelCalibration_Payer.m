function output_vector = ModelCalibration_Payer(gamma,r,discount_dates,discount_curve,P_dates,P_curve,settle,expiries,tenors,freq,vol,init_cond)
% calibrates parameters a and sigma for the selected value of gamma by using Payer swaptions
%
% INPUT
...
MktPrice = @(i) SwaptionPayer_calib(B(expiries(i)*freq:(expiries(i)+tenors(i))*freq),beta(freq*expiries(i):(expiries(i)+tenors(i))*freq-1),payDates(expiries(i)*freq+1:(expiries(i)+tenors(i))*freq),expiriesDates(i),settle,r,vol(i));
...
fun = @(i,parameters) SwaptionPricingModel_Payer(gamma,r,B(expiries(i)*freq:(expiries(i)+tenors(i))*freq),beta(freq*expiries(i):(expiries(i)+tenors(i))*freq-1),parameters(1),parameters(2),times_to_exp(i),cell2mat(deltas(i)),cell2mat(time_intervals(i)));
...
ylabel('sigma');
zlabel('dist');
title('Distance Payer: ', title_string)
